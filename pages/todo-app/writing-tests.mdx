...
```ts showLineNumbers {1} filename="./setupTests.ts"
import '@testing-library/jest-dom/vitest';
```
...
Let us now write a test to check if the user can add a task to the list.

```tsx showLineNumbers {16} filename="./src/App.test.tsx"
import { describe, expect, test } from 'vitest';
import { render, screen } from '@testing-library/react';
...
```tsx showLineNumbers {2, 17-23} filename="./src/App.test.tsx"
import { describe, expect, test } from 'vitest';
...
- First, we are still using the old rendered component from the previous test. This can cause issues if we are not careful. So, we need to render a new component again.

```tsx showLineNumbers {17} filename="./src/App.test.tsx"
import { describe, expect, test } from 'vitest';
import { render, screen, fireEvent } from '@testing-library/react';
...
![Test Fail](./screenshots/writing-tests-3.png)

The error might be imtidiating at first, but it is actually very helpful. It tells us that there multiple elements with the same text. This happened because we did not `clean up` our first test's render. So, the second render had the first test's elements as well. So, we need to clean up the first render and any such subsequent renders.

```tsx showLineNumbers {1-2, 6} filename="./src/App.test.tsx"
...
You can read more about this here and figure out the why and solution. [Here's the link](https://testing-library.com/docs/react-testing-library/api#cleanup)
...
```ts showLineNumbers {2-3, 5} filename="./setupTests.ts"
import '@testing-library/jest-dom/vitest';
...
```tsx showLineNumbers filename="./src/App.test.tsx"
import { describe, expect, test } from 'vitest';
import { render, screen, fireEvent } from '@testing-library/react';
...
This is also good because we are not repeating the same code in every test and even if someone forgets to clean up, it will be taken care of.

Now, run the tests and they should pass. But there is still a problem:

- To greatly amplify the problem, let us make the input field `readonly` and see if our tests still pass.

```tsx showLineNumbers {10} filename="./src/App.tsx"
...
In fact, we could have avoided this problem if we had used a different way of testing user interaction. Even the docs, recommends another library called `user-event` for testing user interaction. [Here's the link to the docs](https://testing-library.com/docs/user-event/intro)
...
Since, user interactions are asynchronous, we need to use `async`, `await`, and `waitFor` from `@testing-library/react` to wait for the changes to take effect. (read more about [async methods](https://testing-library.com/docs/dom-testing-library/api-async))

```tsx showLineNumbers {2-3, 18, 24-25, 27-29} filename="./src/App.test.tsx"
import { describe, expect, test } from 'vitest';
...
Our tests __should fail__. Because we are using `readOnly` on the input field. So, remove the `readOnly` prop from the input field and run the tests again.
...
Now, we have made a decent amount of progress on tests. Remember, where we left our actual business logic? Let us get back to that with `TDD` (Test Driven Development) in the next section.
