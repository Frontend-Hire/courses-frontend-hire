...
<Callout>
Too much test code for a simple implementation? Yes, might seem useless but imagine in the future you had to refactor the code (any software will always grow unless abandoned) and you forgot to clear the input field after adding a task. Your tests would fail and you would know that you need to fix the code.
</Callout>
...
<Callout>
You could also disable the add button if the input field is empty. It is usually considered a bad practice to disable buttons because it affects accessibility. Imagine a user who is using a screen reader and they are not able to add a task because the add button is disabled. They would not know why the button is disabled. So, it's better to not disable buttons and instead show an error message to the user.

I'd suggest reading the article "[The problem with disabled buttons and what to do instead](https://adamsilver.io/blog/the-problem-with-disabled-buttons-and-what-to-do-instead/)" by [Adam Silver](https://adamsilver.io/) to learn more about why you should not disable buttons.
</Callout>
...
At this point your code should be a good match to the branch of the repository: [6-test-driven-development](https://github.com/yaralahruthik/todo-app/tree/6-test-driven-development)