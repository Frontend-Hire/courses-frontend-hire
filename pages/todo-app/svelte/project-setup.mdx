...
We do not have to install ESLint as the SvelteKit installation step already did it us.
...
Install Husky, we pretty much follow the official instructions [here](https://typicode.github.io/husky/get-started.html).
...
This will create a `.husky` directory in the root of our project. Inside this directory, we will find a `pre-commit` file. This is where we will add our commands to lint and format.

Open the `pre-commit` file and add the following:

```sh filename=".husky/pre-commit"
npm run format && npm run lint
```

This will run the `lint` and `format` scripts before we commit our code.
