...
This gives us the following types of `Task` and `Priority`:
...
Putting it all together, we should have the following `main.ts` file:

```ts filename="main.ts" showLineNumbers
import './style.css';
...
You should see the list of tasks rendered on the screen. If you don't, make sure you've saved your changes and that the dev server is running.
...
At this point, your code should be a good match to the branch of the repository: [2-reading-tasks](https://github.com/Frontend-Hire/todo-app-js-tdd-typescript/tree/2-reading-tasks)
